...
#1) Creates a bar graph of the height of the membrane. 
#2) Creates combined graph of height of membrane and percentage of defects in particular region of the membrane in the x-dimension
...
#compiled_data = Path(f"/scratch/local/casakurai/asyn-phase-binding-data/Figures/defect-data-{time}-equil-ext/{configuration}/defect-cut-off-{defect_cut_off}A")
compiled_data = Path(f"/scratch/local/casakurai/asyn-phase-binding-data/Figures/trials")
...
for type in defect_type:
    ...
    plt.savefig(f"{compiled_data}/{type}-compiled-defects-averaged-frames-spatial-weight-{configuration}-{lipid_type}.png")
    plt.close()
...
for defect_type_name in defect_type:
    with plt.style.context(["ctleelab_plothelper.base", "ctleelab_plothelper.light"]):
        fig, ax = ph.fixed_size_subplots(3,2, wmargin = .6, hmargin = 1,  subwidth=.95, subheight=1.1, rmargin_scale = 8)
