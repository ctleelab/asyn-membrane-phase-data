...
for sys, lipid in systems.items():

    analysis = util.analysis_path
    analysis_curv_folder = f"curvature_selection"
    system_analysis_folder = analysis/analysis_curv_folder/ensemble/shape/f"system{sys}-{initial_dim}-{lipid}-flat-NVT"
    ...
    #run production
    prod_mdrun = f" gmx mdrun -nt 64 -update gpu -gpu_id 0  -pin off -v -pinstride 1 -nstlist 100 -deffnm {output_file_prod}"
