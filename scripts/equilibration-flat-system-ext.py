...
for sys in systems:
    ...
    equilibration_number = 6.9
    #previous_equilibration_number = round(equilibration_number - 0.1,1) 
    #gro = system_path/ f"min-{size}"/f"minimization6.1.gro"
    gro = equilibration_path/f"equilibration{equilibration_number}.gro" #set if only running extentensions of equil 
    ...
    while equilibration_number <= 6.9:
        ...
        if equilibration_number == 6.2: #running 6.2 equilibration step seperately from the other because it does not require a .cpt file
            ...
            gro = equilibration_path / f"equilibration{equilibration_number}.gro"
        ...
        if equilibration_number >= 6.7: 
            #re-define files 
            equil_mdp = mdp_path / f"step{equilibration_number}_equilibration.mdp"
            tpr_file = equilibration_path / f"equilibration{equilibration_number}-ext.tpr"
            output_file_equil = equilibration_path / f"equilibration{equilibration_number}-ext"
            #previous_equilibration_number = round(equilibration_number - 0.1,1) 
            cpt_file = equilibration_path / f"equilibration{equilibration_number}.cpt"
            ...
            #run equilibration
            equil_mdrun = f" gmx mdrun -nt 32 -bonded gpu -gpu_id 1 -pin on -pinoffset 0 -v -pinstride 1 -nstlist 100 -deffnm {output_file_equil}"
            subprocess.run(equil_mdrun, shell = True, check = True)
