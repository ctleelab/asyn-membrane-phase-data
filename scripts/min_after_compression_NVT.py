...
# analysis_curv_dir = analysis/analysis_curv_folder/ensemble/f"{system_analysis_folder}"/f"{system_analysis_time_folder}"
# analysis_curv_dir.mkdir(exist_ok = True)

#creates a new .tpr file with DPPC instead of DOPC 
minimization_number = 7.0 

gro_file = system_analysis_folder / f"{final_dim}_compression_{frame}ps_{strain}.gro"
top_file = system_analysis_folder / f"{final_dim}-system.top"
tpr_file = system_analysis_folder / f"minimization{minimization_number}.tpr"
mdp_file_min1 = mdp_path / f"step{minimization_number}_minimization.mdp"
output_file_min1 = system_analysis_folder / f"minimization{minimization_number}"
...
gro_file_min1 = system_analysis_folder / f"minimization7.0.gro"
output_file_min2 = system_analysis_folder / f"minimization{minimization_number}"
tpr_file_min2 = system_analysis_folder / f"minimization{minimization_number}.tpr"
