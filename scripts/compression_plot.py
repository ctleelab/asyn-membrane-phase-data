...
compression_data_analysis = Path("/scratch/local/casakurai/asyn-phase-binding-data/compression-data/analysis")
def compression_plot_set_Lo(system):
    #first grab the length of Lo (which is the average Lx for flat membrane in NPT)
    flat_large_path = Path(f"/scratch/local/casakurai/asyn-phase-binding-data/systems/system{system}-8x8x25/equil-large/")
    ...
    compression_large_path = Path(f"/scratch/local/casakurai/asyn-phase-binding-data/compression-data/system{system}")
    ...
    #from buckled
    compression_strain = (compression_initial_dim_x- terms_compression["Box-X"])/compression_initial_dim_x
    Time_1e6 = terms_compression["Time"]/1000000

    #combine into dataframe 
    df = pd.DataFrame({ "Time_1e6": Time_1e6,"Time": terms_compression["Time"], "Box-X": terms_compression["Box-X"],"Compression strain": compression_strain})
    ...
    compression_strain_adj = (flat_box_x_avg - terms_compression ["Box-X"])/flat_box_x_avg 
    df_new = pd.DataFrame({ "Time_1e6": Time_1e6,"Time": terms_compression["Time"], "Box-X": terms_compression["Box-X"],"Compression strain": compression_strain, "adjusted compression strain": compression_strain_adj})
    adjusted_target_strain =  df_new["adjusted compression strain"].iloc[strain_idx]
    
    #correct target strain 
    idx_at_targert_strain_new = np.argmin(np.abs(df_new["adjusted compression strain"] - target_strain))
    value_idx_at_target_strain_new =  df_new["adjusted compression strain"].iloc[idx_at_targert_strain_new]
    ...
    print(f"The adjusted strain based on Lo of avg Lx of flat NVT would be {adjusted_target_strain}")

    system_strain_comparison[system] = [target_strain, adjusted_target_strain, differenece_between_strains, compression_at_target_strain ]
...
def bar_plot_strains (data):
    systems = ["DOPC", "DPPC", "DOPC-DOPS", "DPPC-DPPS", "DPPC-DPPA", "DOPC-DOPA",]
    differences = []
    new_strains = []
    actual_strains= []
...
def compression_plot_strain(compression_data,system, value_idx_at_targert_strain_new, time_at_target_strain_new):
    ...
    plt.title(f"system{system} target-strain{target_strain} real-strain {value_idx_at_targert_strain_new:.4g} at {time_at_target_strain_new}ps ")
    ...
    plt.tight_layout()
    plt.savefig(compression_data_analysis / f"compression_{target_strain}strain_system{system}_{time_at_target_strain_new}ps.png")
    plt.close()


for system in systems:
    compression_data, value_idx_at_target_strain_new, time_at_target_strain_new = compression_plot_set_Lo(system)
    compression_plot_strain(compression_data, system, value_idx_at_target_strain_new, time_at_target_strain_new)
